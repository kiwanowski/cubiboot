...
#define LDOUBLE_MIN_10_EXP FLT_MIN_10_EXP
#define LDOUBLE_MAX_10_EXP FLT_MAX_10_EXP
...
static void fmtflt(char *, size_t *, size_t, float, int, int, int, int *);
static void printsep(char *, size_t *, size_t);
static int getnumsep(int);
static int getexponent(float);
static int convert(UINTMAX_T, char *, size_t, int, int);
static UINTMAX_T cast(float);
static UINTMAX_T myround(float);
...
static void
fmtflt(char *str, size_t *len, size_t size, float fvalue, int width,
...
{
	float tmp = (value >= 0.0) ? value : -value;
	int exponent = 0;
