...
void pmalloc_dump_stats(pmalloc_t *pm) {
	...
	OSReport(" - freemem: %d\n", pm->freemem);
	OSReport(" - totalmem: %d\n", pm->totalmem);
	OSReport(" - totalnodes: %d (sizeof %d)\n", pm->totalnodes, (int)sizeof(pmalloc_item_t));
	OSReport(" - assigned:\n");
	for(pmalloc_item_t* current = pm->assigned; current != NULL; current=current->next) {
		OSReport("  - (%016llx) %016llx -> %016llx - size: %lld (%ld sys, %d usr)\n", (unsigned int)(char*)current, (unsigned int)(char*)current + sizeof(pmalloc_item_t), (unsigned int)(char*)current + current->size + sizeof(pmalloc_item_t), (unsigned int)(current->size + sizeof(pmalloc_item_t)), sizeof(pmalloc_item_t), current->size);
	} 
	OSReport(" - available:\n");
